...
namespace ItemBrowser.UserInterface.Browser {
	public abstract class GridView : MainSubView {
		...
		private readonly SearchFilter<ObjectDataCD> _searchFilter = new((objectData, names) => {
			var displayName = ObjectUtils.GetLocalizedDisplayName(objectData);
		...
		private void LateUpdate() {
			...
			if ((PrimaryFiltersPanel.HasDynamicFiltersEnabled || Options.Instance.DiscoveryMode) && Time.time >= _refreshedListTime + DynamicFiltersRefreshInterval)
		...
		private void RefreshList(bool preserveScrollPosition) {
			...
			var filteredObjects = UseReverseSorting
				? allObjects
					...
					.OrderByDescending(objectData => Options.Instance.IsFavorited(objectData) ? 1 : 0)
					.ThenByDescending(objectData => CurrentSorter.Function(objectData))
					.ThenByDescending(objectData => _sorters[0].Function(objectData))
					.ToList()
				: allObjects
					.Where(MatchesFilters)
					.OrderByDescending(objectData => Options.Instance.IsFavorited(objectData) ? 1 : 0)
		...
		private bool MatchesFilters(ObjectDataCD objectData) {
			return _searchFilter.Function(objectData)
			       && (!Options.Instance.DiscoveryMode || ObjectUtils.HasBeenDiscovered(objectData, true))
